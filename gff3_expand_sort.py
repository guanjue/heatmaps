		...
		if len(records.split())!=0:
				...
				if roman2arabic_tf =='T':
					...
					if tmp_inf[0] in roman2arabic: ### remove header & convert roman to arabic number
						name=tmp_inf[8].split(';')[1].split('=')[1].split('%')[0]
						if tmp_inf[6] == '+' and int(tmp_inf[3]) - upstream_expan >=0:
							expand_inf = [roman2arabic[tmp_inf[0]] ] + [ int(tmp_inf[3])-upstream_expan-1, int(tmp_inf[4])+downstream_expan, name, '.', tmp_inf[6], int(tmp_inf[4])-int(tmp_inf[3])+1 ]
						elif tmp_inf[6] == '-' and int(tmp_inf[3]) - downstream_expan >=0:
							expand_inf = [roman2arabic[tmp_inf[0]] ] + [ int(tmp_inf[3])-downstream_expan-1, int(tmp_inf[4])+upstream_expan, name, '.', tmp_inf[6], int(tmp_inf[4])-int(tmp_inf[3])+1 ]
						else:
							end_num = end_num + 1
					...
					if tmp_inf[0][0:3] =='chr': ### remove header
						name=tmp_inf[8].split(';')[1].split('=')[1].split('%')[0]
						if tmp_inf[6] == '+' and int(tmp_inf[3]) - upstream_expan >=0:
							expand_inf = [tmp_inf[0], int(tmp_inf[1])-upstream_expan-1, int(tmp_inf[2])+downstream_expan, name, '.', tmp_inf[6], int(tmp_inf[4])-int(tmp_inf[3])+1 ]
						elif tmp_inf[6] == '-' and int(tmp_inf[3]) - downstream_expan >=0:
							expand_inf = [tmp_inf[0], int(tmp_inf[1])-downstream_expan-1, int(tmp_inf[2])+upstream_expan, name, '.', tmp_inf[6], int(tmp_inf[4])-int(tmp_inf[3])+1 ]
						data01.append(expand_inf)
